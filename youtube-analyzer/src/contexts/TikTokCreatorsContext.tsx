// ========== SECAO: CONTEXT TIKTOK OUTROS CRIADORES ==========
...
interface TikTokCreatorsContextType {
  // Busca de criador
  username: string;
...
const TikTokCreatorsContext = createContext<TikTokCreatorsContextType | null>(null);
...
interface TikTokCreatorsProviderProps {
  children: ReactNode;
}

export function TikTokCreatorsProvider({ children }: TikTokCreatorsProviderProps) {
  // Estado da busca
  ...
  const searchCreator = useCallback(async () => {
    ...
    try {
      const result = await fetchTikTokProfile(username.trim(), 500);
      ...
      console.error('[TikTok Creators] Erro ao buscar perfil:', err);
  ...
  return (
    <TikTokCreatorsContext.Provider value={value}>
      {children}
...
export function useTikTokCreators(): TikTokCreatorsContextType {
  const context = useContext(TikTokCreatorsContext);

  if (!context) {
    throw new Error('useTikTokCreators deve ser usado dentro de TikTokCreatorsProvider');
  }
